...
\ThesisFigure{figures/chapter4_td3_pipeline.pdf}{Pipeline huấn luyện, lựa chọn checkpoint và đánh giá TD3}{fig:ch4-pipeline}
\figuresource{Tác giả đề xuất (2026)}

Hình~\ref{fig:ch4-pipeline} nên chia thành ba lane. Lane dữ liệu thể hiện train/validation/test ScenarioBank tách biệt. Lane huấn luyện thể hiện actor, hai critic, replay buffer, dual controller và action decoder. Lane đánh giá thể hiện validation selection, best checkpoint, deterministic test, raw CSV, thống kê và latency. Mũi tên bị chặn từ test về checkpoint cần được vẽ rõ để nhấn mạnh không có test leakage.
