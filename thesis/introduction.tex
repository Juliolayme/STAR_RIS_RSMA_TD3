...
Đa truy cập phân chia tốc độ (Rate-Splitting Multiple Access, RSMA) là một kỹ thuật quản lý nhiễu linh hoạt. Thông điệp của mỗi người dùng được chia thành phần chung và phần riêng. Mọi người dùng giải mã luồng chung trước, loại bỏ luồng này, rồi giải mã luồng riêng của mình. Nhờ đó, RSMA có thể điều chỉnh giữa hai cách xử lý nhiễu: xem nhiễu như tạp âm hoặc giải mã một phần nhiễu \cite{Mao2018RSMABridging,Mao2022RSMAFundamentals}.

Bề mặt thông minh tái cấu hình đồng thời truyền và phản xạ (Simultaneously Transmitting and Reflecting Reconfigurable Intelligent Surface, STAR--RIS) cho phép tạo cả thành phần truyền qua và phản xạ từ cùng một bề mặt. So với bề mặt thông minh tái cấu hình (Reconfigurable Intelligent Surface, RIS) chỉ phản xạ theo mô hình thông dụng, STAR--RIS có thể phục vụ người dùng ở cả hai phía của bề mặt \cite{Xu2021STARRIS,Mu2021STARAided}. Khi kết hợp STAR--RIS với RSMA, hệ thống có thêm nhiều biến điều khiển gồm công suất, tỷ lệ tốc độ chung, hệ số chia năng lượng và pha của từng phần tử.

Bài toán tối ưu đồng thời các biến trên có tính phi lồi và số chiều tăng theo số phần tử STAR--RIS. Các bộ giải lặp có thể đạt nghiệm tốt nhưng phải giải lại cho từng hiện thực kênh. Học tăng cường sâu (Deep Reinforcement Learning, DRL) cung cấp hướng tiếp cận khác: thực hiện chi phí học chủ yếu ở giai đoạn huấn luyện và dùng mạng nơ-ron để tạo quyết định nhanh khi suy luận. Luận văn lựa chọn thuật toán gradient chính sách xác định sâu trì hoãn kép (Twin Delayed Deep Deterministic Policy Gradient, TD3) vì thuật toán này phù hợp với không gian hành động liên tục và có ba cơ chế giảm sai số ước lượng: hai Critic, làm trơn hành động đích và cập nhật Actor trễ \cite{Fujimoto2018TD3}.
...
\begin{enumerate}[label=\arabic*)]
  \item Xây dựng mô hình tín hiệu, kênh hiệu dụng, tốc độ RSMA và mô hình ES của STAR--RIS thống nhất với source code.
  ...
  \item Đánh giá kết quả bằng nhiều seed, thống kê ghép cặp, hiệu chỉnh Holm và benchmark độ trễ trên cùng bộ xử lý trung tâm (Central Processing Unit, CPU).
\end{enumerate}

\section*{4. Câu hỏi nghiên cứu}
\addcontentsline{toc}{section}{4. Câu hỏi nghiên cứu}
\begin{researchquestion}
\textbf{RQ1:} TD3 có tạo được hành động khả thi và duy trì QoS ổn định trên các kịch bản chưa dùng để lựa chọn mô hình hay không?
\end{researchquestion}
\begin{researchquestion}
\textbf{RQ2:} Trong ba implementation DRL được đánh giá, TD3 khác DDPG và PPO như thế nào về tổng tốc độ, QoS, tính ổn định và độ trễ?
\end{researchquestion}
\begin{researchquestion}
\textbf{RQ3:} TD3 tạo ra điểm đánh đổi chất lượng--độ trễ như thế nào khi so với AO--SCA, AO--Grid và AnalyticalRIS?
\end{researchquestion}
\begin{researchquestion}
\textbf{RQ4:} Khi số phần tử STAR--RIS tăng từ 16 lên 128, chất lượng nghiệm, độ tin cậy QoS và độ trễ của các phương pháp thay đổi theo xu hướng nào?
\end{researchquestion}
...
Luận văn đánh giá các implementation và siêu tham số đã công bố trong repository. Các thuật toán DRL dùng cùng ngân sách 100.000 tương tác, cùng ScenarioBank và cùng quy tắc lựa chọn checkpoint; tuy nhiên, chưa thực hiện tìm kiếm siêu tham số với ngân sách bằng nhau cho mọi thuật toán. Do đó, kết luận chỉ áp dụng cho giao thức thực nghiệm đã khai báo.
...
\begin{itemize}
  ...
  \item \textbf{Đánh giá hệ thống:} đo độ trễ của sáu phương pháp trên cùng một CPU runner một luồng.
\end{itemize}

\section*{7. Đóng góp của luận văn}
\addcontentsline{toc}{section}{7. Đóng góp của luận văn}
\begin{enumerate}[label=\arabic*)]
  \item Xây dựng mô hình SISO STAR--RIS--RSMA thống nhất, trong đó mọi phương pháp dùng chung mô-đun vật lý và bộ tính tốc độ.
  \item Xây dựng tham số hóa hành động \texttt{physical\_v3}, bảo đảm công suất và tỷ lệ tốc độ nằm trên simplex, hệ số chia năng lượng thuộc $[0,1]$ và pha nằm trong miền hợp lệ.
  ...
  \item Xây dựng quy trình ScenarioBank tách biệt huấn luyện--xác thực--kiểm thử, lựa chọn checkpoint theo ưu tiên tính khả thi và lưu đầy đủ provenance.
  \item Thực hiện benchmark sáu phương pháp trên năm kích thước STAR--RIS, tám seed cho mỗi thuật toán DRL và 1.000 kịch bản kiểm thử khóa.
...
Ngoài phần Mở đầu, Kết luận, Tài liệu tham khảo và Phụ lục, luận văn gồm bốn chương. Chương 1 trình bày cơ sở lý thuyết và công dụng của các công nghệ RSMA, STAR--RIS và DRL bằng cách diễn giải trực quan, hạn chế công thức. Chương 2 xây dựng mô hình hệ thống và bài toán tối ưu. Chương 3 trình bày quá trình xây dựng TD3, các phương pháp so sánh và giao thức thực nghiệm. Chương 4 báo cáo kết quả mô phỏng, phân tích thống kê, độ trễ, giới hạn và trả lời các câu hỏi nghiên cứu.
