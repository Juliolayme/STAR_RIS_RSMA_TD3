...
\begin{equation}
 ...
 \label{eq:hành động-dimension-ch3}
\end{equation}
...
Đầu ra của Actor không được đưa trực tiếp vào môi trường. Bộ giải mã bộ giải mã vật lý thực hiện
\begin{align}
 ...
 \label{eq:hành động-decoder-ch3}
\end{align}
...
\ThesisFigure{figures/pdf/chapter3_fig02_hành động_decoder.pdf}
{Bộ giải mã hành động chuẩn hóa thành các biến vật lý khả thi}
{fig:ch3-hành động-decoder}[0.94\linewidth]
\figuresource{Tác giả xây dựng từ tham số hóa bộ giải mã vật lý của mô hình triển khai (2026)}

Hình~\ref{fig:ch3-hành động-decoder} cho thấy năm nhánh của hành động. Nhờ tham số hóa này, Actor không cần học lại các ràng buộc hình học cơ bản. QoS chưa được bảo đảm cứng mà được xử lý bằng reward, bộ điều khiển đối ngẫu và lựa chọn checkpoint.
...
Hình~\ref{fig:ch3-td3-architecture} thể hiện ba cải tiến cốt lõi: lấy giá trị nhỏ hơn của hai Critic, làm trơn hành động đích và cập nhật Actor trễ. Replay Buffer cho phép tái sử dụng transition, giúp TD3 có hiệu suất mẫu tốt hơn phương pháp on-chính sách trong cùng ngân sách tương tác.
...
PPO không dùng Replay Buffer và chỉ học từ dữ liệu on-chính sách mới thu thập. Điều này có thể làm PPO cần nhiều tương tác hơn để đạt miền QoS khả thi. Tuy nhiên, PPO có ưu điểm giới hạn mức thay đổi chính sách và đã có tiền lệ trong STAR--RIS--RSMA \cite{Meng2024PPOSTARRSMA}.
