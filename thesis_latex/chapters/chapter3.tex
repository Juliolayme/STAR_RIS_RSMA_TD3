...
\begin{figure}[htbp]
\centering
\resizebox{0.95\textwidth}{!}{\input{figures/fig_ch3_khung_drl}}
...
\begin{figure}[htbp]
\centering
\resizebox{\textwidth}{!}{\input{figures/fig_ch3_vec_to_trang_thai}}
...
\begin{figure}[htbp]
\centering
\resizebox{0.95\textwidth}{!}{\input{figures/fig_ch3_do_sac_softmax}}
...
Khối $a_\eta$ được đưa từ $[-1,1]^K$ về $[0,1]^K$ rồi chiếu Euclid (Euclidean projection) lên đơn hình:
\begin{equation}
\label{eq:eta}
...
\begin{figure}[htbp]
\centering
\resizebox{\textwidth}{!}{\input{figures/fig_ch3_bo_giai_ma}}
...
đối ngẫu (dual variable) $\lambda_t$:
\begin{equation}
...
Mức vi phạm được làm trơn bằng trung bình mũ (exponential moving average):
\begin{equation}
\label{eq:ema}
...
\subsection{Lựa chọn điểm lưu mô hình (checkpoint) theo tập xác thực}
...
\begin{figure}[htbp]
\centering
\resizebox{\textwidth}{!}{\input{figures/fig_ch3_kien_truc_mang}}
...
làm $Q_\psi(s, \mu_\phi(s))$ tăng. Trong cấu hình cuối, hàm mất mát (loss function) Critic dùng hàm Huber (Huber loss)
và gradient được cắt chuẩn (gradient norm clipping) để hạn chế bước cập nhật quá lớn.
...
để lấp bộ nhớ phát lại. Sau đó, DDPG và TD3 dùng hành động Actor cộng nhiễu Gaussian khám phá (exploration noise). Độ lệch chuẩn nhiễu giảm tuyến tính từ $0,12$ xuống $0,03$ trong $100.000$ bước. Với
...
Bộ nhớ phát lại có sức chứa $200.000$ mẫu và lô nhỏ (mini-batch) kích thước $256$. Mỗi mẫu có thể được
...
Trong đó: $\hat{A}_t$ là lợi thế ước lượng (advantage) và $\epsilon$ là biên cắt của PPO. Hàm mục tiêu
...
cách đến điều kiện dừng (stationarity). Cơ chế này làm AO--SCA là một bộ giải cục bộ có kiểm soát tốt hơn
...
Các ngân hàng dùng hạt giống (seed) sinh ngẫu nhiên cố định và có mã kiểm tra (checksum) SHA--256. Một hàm
...
\begin{figure}[htbp]
\centering
\resizebox{0.95\textwidth}{!}{\input{figures/fig_ch3_quy_trinh}}
...
trong các tệp kê khai (manifest), tệp đo thời gian và bảng dữ liệu. Các ngân hàng kênh được kiểm tra
