% =====================================================================
%  Kien truc thu-phat RSMA mot lop cho K UE trong he SISO
%  Bien dich:  xelatex hinh_kientruc.tex
% =====================================================================
\documentclass[11pt]{article}
\usepackage[margin=1cm,paperwidth=22cm,paperheight=14cm]{geometry}
\usepackage{fontspec}
\setmainfont{Liberation Serif}   % Overleaf: doi thanh "Times New Roman"
\usepackage{amsmath}
\usepackage{tikz}
\usetikzlibrary{positioning,fit,shapes.geometric,shapes.symbols,backgrounds,arrows.meta,calc}
\pagestyle{empty}
...
\tikzset{
  ...
  sbox/.style  ={draw,fill=gray!12,align=center,inner sep=2pt,font=\tiny,line width=0.4pt},
  ant/.pic={
...
\newcommand{\lb}[1]{\textcolor{orange!75!black}{$#1$}}
\newcommand{\sg}[1]{\textcolor{blue!60!black}{$#1$}}
...
% thong diep vao
\draw[->] ($(sch.south)+(0,0.85)$)  -- node[above,inner sep=1.5pt]{\lb{W_1}}   ($(spl.north)+(0,0.85)$);
\draw[->] ($(sch.south)+(0,0)$)     -- node[above,inner sep=1.5pt]{\lb{W_k}}   ($(spl.north)+(0,0)$);
\draw[->] ($(sch.south)+(0,-0.85)$) -- node[above,inner sep=1.5pt]{\lb{W_K}}   ($(spl.north)+(0,-0.85)$);
\node[font=\tiny] at (0.8,0.30) {$\vdots$};
\node[font=\tiny] at (0.8,-0.55) {$\vdots$};

% phan chung -> bo ghep
\draw[->] (1.88,2.05) -- node[above,inner sep=1pt,font=\tiny]{\lb{W_{c,1}}} (2.65,2.05);
\draw[->] (1.88,1.50) -- node[above,inner sep=1pt,font=\tiny]{\lb{W_{c,k}}} (2.65,1.50);
\draw[->] (1.88,0.95) -- node[above,inner sep=1pt,font=\tiny]{\lb{W_{c,K}}} (2.65,0.95);

% phan rieng -> bo ma hoa
\draw[->] (1.88,0.10)  -- node[above,inner sep=1pt,font=\tiny]{\lb{W_{p,1}}} (4.32,0.10);
\draw[->] (1.88,-0.40) -- node[above,inner sep=1pt,font=\tiny]{\lb{W_{p,k}}} (4.32,-0.40);
\draw[->] (1.88,-0.90) -- node[above,inner sep=1pt,font=\tiny]{\lb{W_{p,K}}} (4.32,-0.90);
...
\draw[->] (ue1.east) -- (13.6,3.00) node[midway,above,inner sep=1.5pt]{\lb{\widehat{W}_1}};
\node[font=\tiny] at (10.4,2.35) {$\vdots$};

% ============ UE k (chi tiet) ============
\path (10.4,1.85) pic[rotate=180] {ant};
\node[anchor=south east,inner sep=1pt] at (10.42,1.99) {\sg{y_k}};
...
\node[anchor=south,inner sep=2pt] at (16.95,0.68) {\lb{\widehat{W}_k}};

% ============ UE K ============
\path (10.4,-1.80) pic[rotate=180] {ant};
\node[anchor=south west,inner sep=1pt] at (10.28,-1.68) {\sg{y_K}};
...
\draw[->] (ueK.east) -- (13.6,-1.80) node[midway,above,inner sep=1.5pt]{\lb{\widehat{W}_K}};
...
\end{scope}
\node[draw,line width=0.9pt,inner sep=0pt,fit={(-0.9,-2.75)(17.95,3.75)}] {};
\end{tikzpicture}

\vspace{3mm}
{\footnotesize Hình 2.3: Kiến trúc thu--phát RSMA một lớp cho $K$ UE trong hệ SISO\\
Nguồn: Tác giả xây dựng dựa trên [1], [2] (2026)}
