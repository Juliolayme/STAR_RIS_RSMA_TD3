% =====================================================================
%  Hinh 1.2 (ve lai): So sanh RIS phan xa truyen thong va STAR-RIS
%  Bien dich:  xelatex hinh12.tex
% =====================================================================
\documentclass[11pt]{article}
\usepackage[margin=1.4cm,paperwidth=19cm,paperheight=19cm]{geometry}
\usepackage{fontspec}
\setmainfont{Liberation Serif}   % Overleaf: doi thanh "Times New Roman"
\usepackage{amsmath}
\usepackage{tikz}
\usetikzlibrary{positioning,fit,arrows.meta,calc,decorations.pathreplacing,shapes.geometric,patterns}
\pagestyle{empty}
...
\tikzset{
  ...
  note/.style={font=\tiny,align=left,anchor=north west,text width=7.2cm},
}

% mot be mat RIS: cot cac phan tu nho
\newcommand{\risbar}[2]{% #1 = x, #2 = mau
  \foreach \i in {0,...,9}{
...
\begin{scope}[shift={(0,0)}]
  ...
  \node[note] at (0,-0.82)
    {Mỗi phần tử chỉ điều chỉnh pha của tín hiệu phản xạ. Toàn bộ năng lượng
     quay về nửa không gian chứa BS, nên UE nằm sau bề mặt không nhận được
     đường ghép tầng.};
\end{scope}
...
\begin{scope}[shift={(8.2,0)}]
  ...
  \node[font=\scriptsize,anchor=north west] at (0,-0.30)
       {$\phi^T_n=\sqrt{\beta^T_n}\,e^{j\theta^T_n}$, \quad
        ...
        $\beta^T_n+\beta^R_n=1$};
  \node[note] at (0,-0.82)
    {Mỗi phần tử chia năng lượng tới thành một phần truyền qua và một phần
     phản xạ, đồng thời điều chỉnh pha độc lập cho hai phía. Nhờ đó cả bốn UE
     đều nhận được đường ghép tầng.};
\end{scope}

% =====================================================================
% Chi tiet mot phan tu (chung cho ca hai panel)
% =====================================================================
\begin{scope}[shift={(4.3,-4.35)}]
...
\vspace{2mm}
{\footnotesize Hình 1.2: So sánh vùng phục vụ của RIS phản xạ truyền thống và STAR--RIS\\
\textit{Nguồn: Tác giả xây dựng dựa trên [3], [4], [6] (2026)}}
