...
def test_root_readme_uses_constrained_v3_examples() -> None:
    ...
    # Legacy configs may be mentioned only as historical documentation; runnable
    # reproduction examples must not point to them.
...
def test_six_method_readme_distinguishes_gate_from_frozen_count() -> None:
    ...
    assert "minimum of 100 samples" in text
    assert "`--count 500` is an optional extended latency experiment" in text
